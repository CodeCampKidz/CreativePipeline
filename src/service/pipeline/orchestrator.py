...
class Pipeline:
    ...
    #
    #  Stage 2: Legal check
    #  Stage A: Creative direction (1 LLM call per product)
    #  Stage B: Post messages (1 LLM call per ratio)
    #  Stage C: Hero image generation (1 image per language)
    #  Stage D: Resize + text overlay (per ratio × language)
    #  Stage E: Brand compliance check
    # ──────────────────────────────────────────────────────────────────────

    async def _run_legal_check(self) -> LegalCheckResult | None:
        """Stage 2: Run legal content check on campaign message.
    ...
    async def _process_product(
        ...
        *,
        ...
        """Stages A–E for a single product (called in parallel per product).
    ...
    async def _derive_creative_direction(self, product: Product) -> CreativeDirection | None:
        """Stage A: Derive creative direction via the AI Creative Director.
    ...
    async def _generate_post_messages(
        ...
        """Stage B: Generate AI post messages for all languages at a given ratio.
    ...
    async def _generate_hero(
        ...
        *,
        ...
        """Stage C: Generate a culturally tailored hero image for a product + language.
    ...
    async def _process_ratio(
        ...
        *,
        ...
        """Stage D: Resize + text overlay for one aspect ratio across all languages.
    ...
    async def _render_language(
        ...
        """Stage D (continued): Render text overlay for a specific language and save.
    ...
    def _run_brand_check(self, product_slug: str) -> BrandComplianceResult:
        """Stage E: Run brand compliance check for a product's creatives.
    ...
    #  Version History
    #
    #  Loads and formats previous version outputs so that regeneration
    #  produces genuinely different creative angles.
    # ──────────────────────────────────────────────────────────────────────
